...
public class SOFINALLEVEL extends PApplet {
...
int ix=0;
int dy;
float timestep=0.1f;  //variavel de tempo
...
public void setModeMem(int modemem){
  this.modemem = modemem;
}
public void setPI(int N){
  this.N = N;
}
public void setTamMem(int tamanhoMemoria){
...
public void drawpaginas(Vector pag,float x,float y){
      for(int i=0;i<pag.size();i++){
           x=drawpagina((pagina)(pag.get(i)),x,y,i+1);
      }
      ly=ly+2*squaresize;
}
public float drawpagina(pagina p,float x,float y,int ID){
...
public float drawparticao(particao p,float x,float y,int i){
      DecimalFormat df=new DecimalFormat("0.###");
      float largura;
      float largurao;
      if(modedraw==1){
...
public void drawmemoria(Vector memoria,float x,float y){
      float dx=x;
...
public boolean requisicao(process p,int tamanho){
...
public boolean requisicao2(Vector memoria,process p,int tamanho){
     if(modemem==0){
...
public void swapout(float soma){ 
...
class Painel extends JFrame {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "SOFINALLEVEL" };
    if (passedArgs != null) {
